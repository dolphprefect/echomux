...
func (a *Adapter1) GetDevices() ([]*device.Device1, error) {
	...
	for _, path := range list {
		...
		if err != nil {
			// Skip devices whose properties can't be parsed (e.g. BLE devices
			// with uint8-keyed AdvertisingData that go-bluetooth can't handle).
			continue
		}
...
func parseDevice(path dbus.ObjectPath, propsMap map[string]dbus.Variant) (*device.Device1, error) {
	...
	// MapToStruct may fail for devices with non-string-keyed AdvertisingData
	// (e.g. BLE devices with uint8-keyed EIR maps). Ignore the error: NewDevice1
	// already populated Properties via GetProperties(), so the device is valid.
	_ = util.MapToStruct(dev.Properties, propsMap)
