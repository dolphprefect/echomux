...
func MapToStruct(s interface{}, m map[string]dbus.Variant) error {
	for k, v := range m {
		if err := mapStructField(s, k, v); err != nil {
			// Skip fields that can't be decoded (e.g. AdvertisingData with
			// uint8 keys on classic BT devices) so the rest of the struct
			// is still populated.
			log.Debugf("MapToStruct: skipping field %s: %s", k, err)
		}
	}
	return nil
}
